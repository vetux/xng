...
namespace xengine {
    ...
    GBuffer::GBuffer(RenderDevice &device, Vec2i size, int samples)
            : ren(device.getRenderer()), allocator(device.getAllocator()), size(size), samples(samples) {
    ...
    void GBuffer::update(Scene &scene, AssetRenderManager &assetRenderManager) {
        ...
        ren.renderBegin(*renderTarget, RenderOptions({}, renderTarget->getSize(), true));
