...
namespace xng {
    ...
    public:
        static Mat4f getViewMatrix(const Transform &cameraTransform) {
        ...
        void setPerspective(const Transform &cameraTransform,
            ...
            const Mat4f projection = MatrixMath::perspective(fov, aspectRatio, nearClip, farClip);
            const Mat4f view = getViewMatrix(cameraTransform);
        ...
        void setOrthographic(const Transform &cameraTransform,
            ...
            const Mat4f projection = MatrixMath::ortho(left, right, bottom, top, nearClip, farClip);
            const Mat4f view = getViewMatrix(cameraTransform);
            cameraStream.upload(cameraHandle,
                                {
                                    Vec4f(cameraTransform.getPosition().x,
