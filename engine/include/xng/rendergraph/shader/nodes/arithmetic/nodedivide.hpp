...
#ifndef XENGINE_NODEDIVIDE_HPP
#define XENGINE_NODEDIVIDE_HPP

#include "xng/rendergraph/shader/shadernode.hpp"

namespace xng {
    struct NodeDivide final : ShaderNode {
        std::unique_ptr<ShaderNode> left;
        std::unique_ptr<ShaderNode> right;

        NodeDivide(std::unique_ptr<ShaderNode> left, std::unique_ptr<ShaderNode> right)
            : left(std::move(left)),
