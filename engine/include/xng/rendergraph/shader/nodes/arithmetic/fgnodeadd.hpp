...
#ifndef XENGINE_FGNODEADD_HPP
#define XENGINE_FGNODEADD_HPP

#include "xng/rendergraph/shader/fgshadernode.hpp"

namespace xng {
    struct FGNodeAdd final : FGShaderNode {
        std::unique_ptr<FGShaderNode> left;
        std::unique_ptr<FGShaderNode> right;

        FGNodeAdd(std::unique_ptr<FGShaderNode> left, std::unique_ptr<FGShaderNode> right)
            : left(std::move(left)),
