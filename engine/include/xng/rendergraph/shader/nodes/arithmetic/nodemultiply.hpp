...
#ifndef XENGINE_NODEMULTIPLY_HPP
#define XENGINE_NODEMULTIPLY_HPP

#include "xng/rendergraph/shader/shadernode.hpp"

namespace xng {
    struct NodeMultiply final : ShaderNode {
        std::unique_ptr<ShaderNode> left;
        std::unique_ptr<ShaderNode> right;

        NodeMultiply(std::unique_ptr<ShaderNode> left, std::unique_ptr<ShaderNode> right)
            : left(std::move(left)),
...
#endif //XENGINE_NODEMULTIPLY_HPP