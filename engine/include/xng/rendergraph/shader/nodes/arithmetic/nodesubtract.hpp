...
#ifndef XENGINE_NODESUBTRACT_HPP
#define XENGINE_NODESUBTRACT_HPP

#include "xng/rendergraph/shader/shadernode.hpp"

namespace xng {
    struct NodeSubtract final : ShaderNode {
        std::unique_ptr<ShaderNode> left;
        std::unique_ptr<ShaderNode> right;

        NodeSubtract(std::unique_ptr<ShaderNode> left, std::unique_ptr<ShaderNode> right)
            : left(std::move(left)),
...
#endif //XENGINE_NODESUBTRACT_HPP
