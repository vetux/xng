...
#ifndef XENGINE_NODEAND_HPP
#define XENGINE_NODEAND_HPP

#include "xng/rendergraph/shader/shadernode.hpp"

namespace xng {
    struct NodeAnd final : ShaderNode {
        std::unique_ptr<ShaderNode> left;
        std::unique_ptr<ShaderNode> right;

        NodeAnd(std::unique_ptr<ShaderNode> left, std::unique_ptr<ShaderNode> right)
            : left(std::move(left)),
