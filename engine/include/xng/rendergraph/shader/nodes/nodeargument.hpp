...
#ifndef XENGINE_NODEARGUMENT_HPP
...
namespace xng {
    struct NodeArgument final : ShaderNode {
        ...
        explicit NodeArgument(const std::string &argument_name)
        ...
        std::unique_ptr<ShaderNode> copy() const override {
            return std::make_unique<NodeArgument>(argumentName);
        }
    };
}

#endif //XENGINE_NODEARGUMENT_HPP