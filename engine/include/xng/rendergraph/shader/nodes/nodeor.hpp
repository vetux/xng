...
#ifndef XENGINE_NODEOR_HPP
#define XENGINE_NODEOR_HPP

#include "xng/rendergraph/shader/shadernode.hpp"

namespace xng {
    struct NodeOr final : ShaderNode {
        std::unique_ptr<ShaderNode> left;
        std::unique_ptr<ShaderNode> right;

        NodeOr(std::unique_ptr<ShaderNode> left, std::unique_ptr<ShaderNode> right)
            : left(std::move(left)),
