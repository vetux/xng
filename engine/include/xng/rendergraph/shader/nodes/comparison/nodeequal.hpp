...
#ifndef XENGINE_NODEEQUAL_HPP
#define XENGINE_NODEEQUAL_HPP

#include "xng/rendergraph/shader/shadernode.hpp"

namespace xng {
    struct NodeEqual final : ShaderNode {
        std::unique_ptr<ShaderNode> left;
        std::unique_ptr<ShaderNode> right;

        NodeEqual(std::unique_ptr<ShaderNode> left, std::unique_ptr<ShaderNode> right)
            : left(std::move(left)),
