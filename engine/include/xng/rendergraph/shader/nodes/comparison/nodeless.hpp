...
#ifndef XENGINE_NODELESS_HPP
#define XENGINE_NODELESS_HPP

#include "xng/rendergraph/shader/shadernode.hpp"

namespace xng {
    struct NodeLess final : ShaderNode {
        std::unique_ptr<ShaderNode> left;
        std::unique_ptr<ShaderNode> right;

        NodeLess(std::unique_ptr<ShaderNode> left, std::unique_ptr<ShaderNode> right)
            : left(std::move(left)),
