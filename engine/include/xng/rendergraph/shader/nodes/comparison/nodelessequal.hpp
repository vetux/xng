...
#ifndef XENGINE_NODELESSEQUAL_HPP
#define XENGINE_NODELESSEQUAL_HPP

#include "xng/rendergraph/shader/shadernode.hpp"

namespace xng {
    struct NodeLessEqual final : ShaderNode {
        std::unique_ptr<ShaderNode> left;
        std::unique_ptr<ShaderNode> right;

        NodeLessEqual(std::unique_ptr<ShaderNode> left, std::unique_ptr<ShaderNode> right)
            : left(std::move(left)),
...
#endif //XENGINE_NODELESSEQUAL_HPP