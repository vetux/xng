...
namespace xng
{
    ...
    void CanvasRenderPass::create(RenderGraphBuilder& builder)
    {
        ...
        shaderBuffer = builder.createShaderBuffer(sizeof(BufferData::CPU));
        ...
        for (auto& canvas : canvases)
        {
            for (auto& paintCommand : canvas.getPaintCommands())
            {
                ...
                for (auto& glyph : text.text.glyphs)
                {
                    ...
                    data.uvOffset_uvScale = Vec4f(0, 0, 1, 1);

                    auto atlasScale = (atlasTex.size.convert<float>() /
                        TextureAtlas::getResolutionLevelSize(
                            atlasTex.level).convert<float>());
                    data.atlasScale_texSize = Vec4f(atlasScale.x, atlasScale.y, atlasTex.size.x, atlasTex.size.y);

                    ctx.uploadBuffer(shaderBuffer,
                                     reinterpret_cast<const uint8_t*>(&data),
                                     sizeof(BufferData::CPU),
                                     0);
            ...
            case Paint::PAINT_IMAGE:
                {
                    ...
                    data.uvOffset_uvScale = Vec4f(uvOffset.x, uvOffset.y, uvScale.x, uvScale.y);

                    auto atlasScale = (atlasTex.size.convert<float>() /
                        TextureAtlas::getResolutionLevelSize(
                            atlasTex.level).convert<float>());
                    data.atlasScale_texSize = Vec4f(atlasScale.x,
                                                    atlasScale.y,
                                                    static_cast<float>(atlasTex.size.x),
                                                    static_cast<float>(atlasTex.size.y));
            ...
            case Paint::PAINT_TEXT:
                {
                    ...
                    data.useCustomTexture = 1;
                    data.texAtlasIndex = -1;
                    data.texAtlasLevel = -1;
                    data.colorMixFactor = 0;
                    data.alphaMixFactor = 0;
                    data.colorFactor = 0;
                    data.texFilter = 0;
